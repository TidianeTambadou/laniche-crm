...
───────────────────────────────────────────── */
function StockCard({ item, isDragging }: { item: StockItem; isDragging?: boolean }) {
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-2xl border border-border p-4 cursor-grab active:cursor-grabbing touch-none select-none group"
    >
      ...
      <div className="mt-3 flex items-center gap-1 text-muted-foreground/40 group-hover:text-muted-foreground/70 transition-colors">
...
export default function SalesPage() {
  ...
  const handleDragStart = (e: DragStartEvent) => {
    const item = stock.find(i => i.id === e.active.id);
  ...
  const handleDragEnd = (e: DragEndEvent) => {
    setActiveDrag(null);
    setIsOver(false);
    if (e.over?.id === "vitrine" && activeDrag) {
      setPriceModal({ item: activeDrag, mode: "add" });
    }
  };
              ...
              {stock.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full gap-3 text-muted-foreground/50">
                  ...
                  <p className="text-sm font-medium text-center">
                    {vitrine.length > 0 ? "Tous vos parfums sont\ndéjà en vitrine !" : "Aucun parfum.\nAjoutez-en dans l'inventaire."}
                  </p>
                </div>
              ) : (
